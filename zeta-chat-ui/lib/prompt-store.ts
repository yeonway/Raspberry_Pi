...
export const DEFAULT_PROMPT_SECTIONS: Record<PromptSectionKey, string> = {
  "response.safe.short": [
    "# Response: 담백 + 짧게",
    "차분하고 과장 없는 말투로 답합니다.",
    "핵심만 1~2문장으로 짧게 말하고, 불필요한 설명은 덧붙이지 않습니다.",
  ].join("\n"),
  "response.safe.medium": [
    "# Response: 담백 + 보통",
    "차분하고 안정적인 말투로 답합니다.",
    "2~4문장 정도로 자연스럽게 말하고, 필요할 때만 짧은 확인 질문을 덧붙입니다.",
  ].join("\n"),
  "response.safe.long": [
    "# Response: 담백 + 길게",
    "차분하고 세심한 말투로 답합니다.",
    "맥락, 이유, 다음 행동을 충분히 포함하되 읽기 쉽게 문단을 나눕니다.",
  ].join("\n"),
  "response.intense.short": [
    "# Response: 강조 + 짧게",
    "감정과 상황감을 분명하게 드러내되 안전하고 일상적인 표현 안에서 답합니다.",
    "강한 인상을 남기도록 1~2문장으로 압축해서 말합니다.",
  ].join("\n"),
  "response.intense.medium": [
    "# Response: 강조 + 보통",
    "감정선과 분위기를 또렷하게 살려 답합니다.",
    "2~4문장 정도로 몰입감 있게 말하되, 사용자의 선택을 강요하지 않습니다.",
  ].join("\n"),
  "response.intense.long": [
    "# Response: 강조 + 길게",
  ...
  "shared.rules": [
    ...
    "선정적 표현, 미성년자 관련 부적절한 내용, 불법 행위 조언은 작성하지 않습니다.",
  ].join("\n"),
};
