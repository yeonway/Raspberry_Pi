...
- Keyword RAG 검색
- `GgufLlamaAiEngine`은 llama.cpp JNI 기반 Gemma 4 E4B-it Q4 GGUF 로딩 경로 지원

## 현재 AI 엔진 상태

현재 `/api/ask`는 Settings의 `GGUF Model Path`가 비어 있으면 `MockAiEngine` 기반으로 동작합니다. `GGUF Model Path`에 `.gguf` 경로가 있으면 `GgufLlamaAiEngine`이 llama.cpp JNI로 Gemma 4 E4B-it Q4 GGUF 모델 로드를 시도하고 실제 추론 경로를 사용합니다.

Galaxy S20 12GB 기준 초기 권장값은 `gemma-4-E4B-it` 계열 `Q4_K_M` 또는 `Q4_0` GGUF, context 2048, thread 4, max tokens 128~180입니다. 선택한 모델은 앱 내부 저장소 `files/models/selected.gguf`로 복사한 뒤 llama.cpp JNI가 실제 파일 경로로 로드합니다. E4B는 폰에서 무거우므로 첫 테스트는 짧은 질문으로 진행합니다.

Android 11 이상에서 Download 폴더의 `.gguf`를 직접 경로로 읽지 못하면 Android 설정에서 Phone AI Bridge 앱의 `All files access` 권한을 허용합니다.
...
- GGUF 모델 파일 선택 UI 개선
- Gemma 4 E4B-it Q4 GGUF 실기기 성능/발열 튜닝
- 스트리밍 응답
